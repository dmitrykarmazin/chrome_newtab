<template>
   <div class="page-wrap">
      <page-item v-for="(page, index) in pages" :page="page"
      v-on:remove = "removePage"
      v-on:addPage = "addPage(index)"
      v-on:editPage = "editPage(page)"
      :key="index">
      </page-item>
      <modal :active.sync="modal_active" :props="compProps" 
              v-on:close="modal_active = false" :component="component">
      </modal>
   </div> 
   
</template>
<script>
import pageItem from './page-item.vue'
//import ls from '../utils/localStorage.js'
import modal from '../../shared/components/modal.vue'

var pages = [
        {
          title:'Youtube',
          link:'https://www.youtube.com',
          page_icon:'data:image/svg+xml;utf8;base64,',
          color:''
        },
        {
          title:'Twitter',
          link:'https://twitter.com/',
          page_icon:'data:image/svg+xml;utf8;base64,',
          color:''
        },
        {
          title:'Facebook',
          link:'https://www.facebook.com/',
          page_icon:'data:image/svg+xml;utf8;base64,',
          color:''
        }, 
        {
          title:'',
          link:'',
          page_icon:'',
          color:''
        }, 
        {
          title:'',
          link:'',
          page_icon:'',
          color:''
        }, {
          title:'',
          link:'',
          page_icon:'',
          color:''
        }, {
          title:'',
          link:'',
          page_icon:'',
          color:''
        }, 
        {
          title:'',
          link:'',
          page_icon:'',
          color:''
        }, 
        {
          title:'',
          link:'',
          page_icon:'',
          color:''
        } 
]
//ls.set('pages', pages)

export default {
  name:'pageGrid',
  data(){
    return{
      defaultPageCoun:9,
      pages:pages,
      modal_active:false,
      component:'',
      compProps:{}
    }
  },
  components:{
    pageItem,
    modal
  },
  methods:{
    removePage(page){
      const pageIndex = this.pages.indexOf(page)
      console.log(pageIndex);
    }, 
    addPage(){
      this.component = 'add-page'
      this.modal_active = true
    },
    editPage(page){
      this.component = 'edit-page'
      this.compProps = { page }
      this.modal_active = true
    }
  }
}
</script>
<style lang="scss">
  .page-wrap{
    display: flex;
    width: 25%;
    justify-content: space-around;
    align-items: center;
    flex-wrap: wrap;
    //background-color: rgba(53, 31, 68, 0.3);
  }
</style>
